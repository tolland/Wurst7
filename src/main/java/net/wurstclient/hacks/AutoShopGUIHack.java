...
public final class AutoShopGUIHack extends Hack implements UpdateListener
{
		...
		if(match.isEmpty())
		{
			if(debugMode.isChecked())
			{
				...
				config.getTargets().stream().filter(ShopTarget::isEnabled)
					.forEach(
						t -> debugLog("  - Looking for: " + t.getNpcName()));
			}
			return;
		}
	...
	private void openShop()
	{
		...
		// Check range
		if(player.distanceToSqr(targetNPC) > range.getValueSq())
		{
		...
		InteractionResult actionResult =
			gm.interact(player, targetNPC, hitResult, hand);
		
		// @TODO
		// this signature is not available in 26.2 what is it doing
		// if(!actionResult.consumesAction())
		// gm.interact(player, targetNPC, hand);
		...
		NavigationStep navStep)
	{
		...
		if(debugMode.isChecked())
		{
			...
			if(!stack.isEmpty())
			{
				...
				debugLog("Clicking: " + itemName + " (" + navStep.getClickType()
					+ " click)");
			}
		}
		...
		MC.gameMode.handleContainerInput(screen.getMenu().containerId,
			slot.index, mouseButton, ContainerInput.PICKUP, MC.player);
	}
		...
		int slotIndex)
	{
		...
		MC.gameMode.handleContainerInput(screen.getMenu().containerId,
			slot.index, 0, ContainerInput.PICKUP, MC.player);
	}
