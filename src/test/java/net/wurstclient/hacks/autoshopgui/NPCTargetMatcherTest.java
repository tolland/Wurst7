...
class NPCTargetMatcherTest
{
	...
	private ShopTarget createTarget(String npcName, String itemName,
		boolean enabled)
	{
		List<NavigationStep> nav = List.of(
			new NavigationStep(10, NavigationStep.ClickType.LEFT));
	...
	void testFindFirstMatch_SimpleMatch()
	{
		...
		List<ShopTarget> targets = List.of(createTarget("Farm Shop", "Wheat", true));
	...
	void testFindFirstMatch_PartialNameMatch()
	{
		...
		List<ShopTarget> targets = List.of(createTarget("farming", "Wheat", true));

		Optional<Match> result = NPCTargetMatcher.findFirstMatch(npcs, targets);

		assertTrue(result.isPresent(), "Should match partial name (case-insensitive)");
	...
	{
		// Multiple NPCs in range, should return first match for first enabled target
		List<NPCInfo> npcs = List.of(
			new NPCInfo("Wood Shop", 5.0),
			new NPCInfo("Farm Shop", 10.0),
	...
	void testFindFirstMatch_MultipleTargets_FirstEnabledMatches()
	{
		...
		List<ShopTarget> targets = List.of(
			createTarget("Wood Shop", "Oak Log", true),  // No NPC for this
	...
	void testFindFirstMatch_DisabledTargetsIgnored()
	{
		...
		List<NPCInfo> npcs = List.of(new NPCInfo("Farm Shop", 10.0));
		List<ShopTarget> targets = List.of(
			createTarget("Farm Shop", "Wheat", false),  // Disabled
	...
	void testFindFirstMatch_AllTargetsDisabled()
	{
		...
		List<NPCInfo> npcs = List.of(new NPCInfo("Farm Shop", 10.0));
		List<ShopTarget> targets = List.of(
			createTarget("Farm Shop", "Wheat", false),
			createTarget("Farm Shop", "Sugar Cane", false)
		);

		Optional<Match> result = NPCTargetMatcher.findFirstMatch(npcs, targets);

		assertFalse(result.isPresent(), "Should not find a match when all targets disabled");
	}
	...
	void testFindFirstMatch_EmptyTargetList()
	{
		...
		assertFalse(result.isPresent(), "Should not find a match with no targets");
	}
	...
	void testFindFirstMatch_PriorityOrder()
	{
		...
		List<ShopTarget> targets = List.of(
			createTarget("Mining Shop", "Iron", true),    // First enabled
	...
	void testFindFirstMatch_MultipleNPCsSameTarget()
	{
		...
		List<NPCInfo> npcs = List.of(
			new NPCInfo("Farming Shop 1", 10.0),
	...
	void testFindFirstMatch_ComplexScenario()
	{
		...
		List<NPCInfo> npcs = List.of(
			new NPCInfo("Farmer Bob - farming specialist", 10.0)
		);
		List<ShopTarget> targets = List.of(
			createTarget("Farm Shop", "Sugar Cane", true),  // Not present
		...
		assertEquals("Farmer Bob - farming specialist", result.get().getNpc().getName());
	...
	void testFindFirstMatch_PartialNameEdgeCases()
	{
		...
		List<NPCInfo> npcs = List.of(
			new NPCInfo("ShopGUI Farmer", 10.0)
		);

		// Should match
		assertTrue(NPCTargetMatcher.findFirstMatch(npcs,
			List.of(createTarget("Farmer", "Wheat", true))).isPresent());
		assertTrue(NPCTargetMatcher.findFirstMatch(npcs,
			List.of(createTarget("ShopGUI", "Wheat", true))).isPresent());
		assertTrue(NPCTargetMatcher.findFirstMatch(npcs,
			List.of(createTarget("GUI Far", "Wheat", true))).isPresent());

		// Should not match
		assertFalse(NPCTargetMatcher.findFirstMatch(npcs,
			List.of(createTarget("Mining", "Iron", true))).isPresent());
	}
